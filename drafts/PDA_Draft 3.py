...
with open('output/N2_'+str(am_N2)+'/H2_density.csv', 'w', newline='\n') as H2_array, open('output/N2_'+str(am_N2)+'/N2_density.csv', 'w', newline='\n') as N2_array, open('output/N2_'+str(am_N2)+'/total_gas_density.csv', 'w', newline='\n') as total_gas_array:
	...
	for ii, t in enumerate(ts.t):
		...
		gas_dens_tot[ii] = 0.5*e_dens_tot[ii]*(1/(1-(am_N2/100 -(inj_thres-1)*(am_N2/100))*np.exp(-((c*t)-mu)**2/(2*sigma**2))))

		gas_dens_N2[ii] = (gas_dens_tot[ii]-0.5*e_dens_tot[ii])*(1/(2-inj_thres))*np.exp(-((c*t)-mu)**2/(2*sigma**2)) 

		#gas_dens_H2[ii] = gas_dens_tot[ii]-gas_dens_N2[ii]

		gas_dens_H2[ii] = 0.5*e_dens_tot[ii] - (inj_thres-1)*gas_dens_N2[ii]		
		
		#gas_dens_H2[ii] = (0.5*e_dens_tot[ii]-(inj_thres-1)*gas_dens_tot[ii])/(2-inj_thres)		
...
ax.plot(ts.t*c*1e3, gas_dens_N2*2*(inj_thres-1)+gas_dens_H2*2, color='red', label="$Test$")
